...
class Voice():
    ...
    async def musicYoutube(self, ctx, searchTerm=None):
        ...
        try:
            ...
            await self.bot.say("There were no video results for that link.")
            self.voice[ctx.message.server.id][1] = None
            return
    ...
    async def play(self, ctx, *, searchTerm : str):
        await self.musicYoutube(ctx, searchTerm)


    @commands.command(pass_context=True,hidden=True)
    async def scream(self, ctx):
        await self.musicYoutube(ctx, "incoherent screaming")


    @commands.command(pass_context=True,hidden=True)
    async def fuckmyass(self, ctx):
        await self.musicYoutube(ctx, "mujsPpzx2Sc")


    @commands.command(pass_context=True,hidden=True)
    async def rickroll(self, ctx):
        rickOrNever = ['dQw4w9WgXcQ','lXMskKTw3Bc']
        toPlay = random.choice(rickOrNever)
        await self.musicYoutube(ctx, toPlay)


    @commands.command(pass_context=True,hidden=True)
    async def badopinion(self, ctx):
        await self.musicYoutube(ctx, "hitler did nothing wrong")


    @commands.command(pass_context=True,hidden=True)
    async def soviet(self, ctx):
        await self.musicYoutube(ctx, "U06jlgpMtQs")


    @commands.command(pass_context=True,hidden=True)
    async def wet(self, ctx):
        await self.musicYoutube(ctx, "Hit me with your wet dick")


    @commands.command(pass_context=True,hidden=True)
    async def stfu(self, ctx):
        await self.musicYoutube(ctx, "OLpeX4RRo28") 


    @commands.command(pass_context=True,hidden=True)
    async def kim(self, ctx):
        await self.musicYoutube(ctx, "kim possible theme")
    ...
    async def putin(self, ctx):
        await self.musicYoutube(ctx, "PUTIN IS NUMBER ONE GREATEST PRESIDENT SONG")


    @commands.command(pass_context=True,hidden=True)
    async def bike(self, ctx):
        await self.musicYoutube(ctx, "nigger stole my bike")


    # @commands.command(pass_context=True,hidden=True)
    # async def succ(self, ctx):
    #     await self.musicYoutube(ctx, "succ")


    @commands.command(pass_context=True,hidden=True)
    async def shia(self, ctx):
        await self.musicYoutube(ctx, "\"Shia LaBeouf\" Live - Rob Cantor")


    @commands.command(pass_context=True,hidden=True)
    async def sorry(self, ctx):
        await self.musicYoutube(ctx, "liberal democrats apology song autotune")


    @commands.command(pass_context=True,hidden=True)
    async def same(self, ctx):
        await self.musicYoutube(ctx, "Hey how you doing. Well I'm doing just fine. I lied I'm dying inside vine by Choonie")


    @commands.command(pass_context=True,hidden=True)
    async def spain(self, ctx):
        await self.musicYoutube(ctx, "9-_2QpbXMbw")
